"""Module to put any functions that are referred to in the "callables" section of VecReg.yaml"""
...
# Original source at L1216 of <nipype-install>/interfaces/fsl/dti.py
...
# Original source at L1205 of <nipype-install>/interfaces/fsl/dti.py
def _list_outputs(inputs=None, stdout=None, stderr=None, output_dir=None):
    outputs = {}
    outputs["out_file"] = inputs.out_file
    if (outputs["out_file"] is attrs.NOTHING) and (inputs.in_file is not attrs.NOTHING):
        pth, base_name = os.path.split(inputs.in_file)
        outputs["out_file"] = _gen_fname(
            base_name,
            cwd=os.path.abspath(pth),
            suffix="_vreg",
            inputs=inputs,
            stdout=stdout,
            stderr=stderr,
