"""Module to put any functions that are referred to in the "callables" section of FILMGLS.yaml"""
...
def param_estimates_callable(output_dir, inputs, stdout, stderr):
    ...
    return outputs["param_estimates"]
...
# Original source at L841 of <nipype-install>/interfaces/fsl/model.py
...
# Original source at L827 of <nipype-install>/interfaces/fsl/model.py
def _get_pe_files(cwd, inputs=None, stdout=None, stderr=None, output_dir=None):
    files = None
    if inputs.design_file is not attrs.NOTHING:
        fp = open(inputs.design_file, "rt")
        for line in fp.readlines():
...
# Original source at L860 of <nipype-install>/interfaces/fsl/model.py
def _list_outputs(inputs=None, stdout=None, stderr=None, output_dir=None):
    ...
    pe_files = _get_pe_files(
        results_dir, inputs=inputs, stdout=stdout, stderr=stderr, output_dir=output_dir
    )
    if pe_files:
