"""Module to put any functions that are referred to in the "callables" section of PRELUDE.yaml"""
...
def unwrapped_phase_file_callable(output_dir, inputs, stdout, stderr):
    outputs = _list_outputs(
        output_dir=output_dir, inputs=inputs, stdout=stdout, stderr=stderr
    )
    return outputs["unwrapped_phase_file"]


IFLOGGER = logging.getLogger("nipype.interface")


# Original source at L2115 of <nipype-install>/interfaces/fsl/preprocess.py
...
# Original source at L2102 of <nipype-install>/interfaces/fsl/preprocess.py
def _list_outputs(inputs=None, stdout=None, stderr=None, output_dir=None):
    outputs = {}
    out_file = inputs.unwrapped_phase_file
    if out_file is attrs.NOTHING:
        if inputs.phase_file is not attrs.NOTHING:
            out_file = _gen_fname(
                inputs.phase_file,
                suffix="_unwrapped",
                inputs=inputs,
        ...
        elif inputs.complex_phase_file is not attrs.NOTHING:
            out_file = _gen_fname(
                inputs.complex_phase_file,
