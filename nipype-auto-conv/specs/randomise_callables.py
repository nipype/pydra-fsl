"""Module to put any functions that are referred to in the "callables" section of Randomise.yaml"""
...
def f_corrected_p_files_callable(output_dir, inputs, stdout, stderr):
    ...
    return outputs["f_corrected_p_files"]
...
def t_corrected_p_files_callable(output_dir, inputs, stdout, stderr):
    ...
    return outputs["t_corrected_p_files"]
...
# Original source at L2322 of <nipype-install>/interfaces/fsl/model.py
def _list_outputs(inputs=None, stdout=None, stderr=None, output_dir=None):
    ...
    if prefix:
        outputs["t_p_files"] = glob(
            _gen_fname(
                "%s_%s_p_tstat*" % (inputs.base_name, prefix),
                inputs=inputs,
                stdout=stdout,
                stderr=stderr,
                output_dir=output_dir,
        ...
        outputs["t_corrected_p_files"] = glob(
            _gen_fname(
                "%s_%s_corrp_tstat*.nii" % (inputs.base_name, prefix),
                inputs=inputs,
                stdout=stdout,
                stderr=stderr,
                output_dir=output_dir,
        ...
        outputs["f_p_files"] = glob(
            _gen_fname(
                "%s_%s_p_fstat*.nii" % (inputs.base_name, prefix),
                inputs=inputs,
                stdout=stdout,
                stderr=stderr,
                output_dir=output_dir,
        ...
        outputs["f_corrected_p_files"] = glob(
            _gen_fname(
                "%s_%s_corrp_fstat*.nii" % (inputs.base_name, prefix),
                inputs=inputs,
                stdout=stdout,
                stderr=stderr,
                output_dir=output_dir,
