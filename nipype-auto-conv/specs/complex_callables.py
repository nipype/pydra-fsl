...
def magnitude_out_file_callable(output_dir, inputs, stdout, stderr):
    ...
    return outputs["magnitude_out_file"]


def phase_out_file_callable(output_dir, inputs, stdout, stderr):
    outputs = _list_outputs(
        output_dir=output_dir, inputs=inputs, stdout=stdout, stderr=stderr
    )
    return outputs["phase_out_file"]


def real_out_file_callable(output_dir, inputs, stdout, stderr):
    outputs = _list_outputs(
        output_dir=output_dir, inputs=inputs, stdout=stdout, stderr=stderr
    )
