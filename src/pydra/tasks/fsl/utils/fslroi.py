...
class FSLROISpec(ShellSpec):
    ...
    output_image: str = field(
        metadata={"help_string": "output image", "argstr": "", "output_file_template": "{input_image}_roi"}
    )

    x_min: int = field(
        metadata={"help_string": "start of ROI in x (0-based indexing)", "argstr": "", "requires": _requires}
    )

    x_size: int = field(
        metadata={"help_string": "size of ROI in x (-1 for maximum)", "argstr": "", "requires": {"x_min"}}
    )

    y_min: int = field(
        metadata={"help_string": "start of ROI in y (0-based indexing)", "argstr": "", "requires": _requires}
    )

    y_size: int = field(
        metadata={"help_string": "size of ROI in y (-1 for maximum)", "argstr": "", "requires": {"y_min"}}
    )

    z_min: int = field(
        metadata={"help_string": "start of ROI in z (0-based indexing)", "argstr": "", "requires": _requires}
    )

    z_size: int = field(
        metadata={"help_string": "size of ROI in z (-1 for maximum)", "argstr": "", "requires": {"z_min"}}
    )

    t_min: int = field(default=0, metadata={"help_string": "start of ROI in t (0-based indexing)", "argstr": ""})
