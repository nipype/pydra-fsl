...
@shell.define
class TemporalFilter(shell.Task["TemporalFilter.Outputs"]):
    """
    Examples
    -------
    ...
    executable = "fslmaths"
    lowpass_sigma: float = shell.arg(
        help="lowpass filter sigma (in volumes)",
        argstr="{lowpass_sigma:.6}",
        position=5,
        default=-1,
    )
    highpass_sigma: float = shell.arg(
        help="highpass filter sigma (in volumes)",
