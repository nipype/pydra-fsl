...
@shell.define(xor=[["operand_file", "operand_value"]])
class BinaryMaths(shell.Task["BinaryMaths.Outputs"]):
    """
    Examples
    -------
    ...
    operation: ty.Any = shell.arg(
        help="operation to perform", argstr="-{operation}", position=4
    )
    operand_file: File | None = shell.arg(
        help="second image to perform operation with",
        argstr="{operand_file}",
        position=5,
    )
    operand_value: float | None = shell.arg(
        help="value to perform operation with", argstr="{operand_value:.8}", position=5
    )
