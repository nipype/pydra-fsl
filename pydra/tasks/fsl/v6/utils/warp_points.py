...
class WarpPoints(shell.Task["WarpPoints.Outputs"]):
    """
    Examples
    -------
    ...
    >>> task = WarpPoints()
    >>> task.src_file = File.mock()
    >>> task.dest_file = Nifti1.mock("T1.nii")
    ...
    src_file: File = shell.arg(
        help="filename of source image", argstr="-src {src_file}"
    )
    dest_file: Nifti1 = shell.arg(
        help="filename of destination image", argstr="-dest {dest_file}"
    )
